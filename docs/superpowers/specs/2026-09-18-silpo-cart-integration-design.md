...
## Доповнення 2026-09-18: позначка «у кошику Сільпо» і підказка про оплату

**Затверджено користувачем:** варіант з підказкою і підтвердженням (не автоматичні галочки).

- `ShoppingListItem.silpo?: { productId, productName, quantity, addedAt }` — ставиться сервером у
  `POST /api/silpo/cart/add` (клієнт передає `itemId` разом із товаром). `mergeShoppingItems`
  переносить позначку при перебудові списку. UI: бейдж «🛒 у кошику Сільпо» замість підказки
  страв; кнопка «Замовити в Сільпо (N)» рахує лише непозначені та некуплені продукти.
- `GET /api/silpo/orders/check` — викликається при відкритті списку, лише якщо є позначені й
  некуплені продукти. Читає `silpo_get_my_online_orders(limit 10)` і поточний кошик; повертає
  `suggestions[]` (замовлення, створене після `addedAt`, з `products[].id === productId` і
  `removed: false`; кожен продукт лише в найновішому замовленні) без замовлень із
  `silpoOrdersHandled` на документі списку; попутно знімає позначки з продуктів, яких немає ні в
  кошику, ні в замовленнях (юзер видалив у Сільпо), і повертає їх у `untagged[]`.
- `POST /api/silpo/orders/confirm { orderId, itemIds, action: 'confirm' | 'dismiss' }` —
  `confirm` ставить `isPurchased: true, purchasedPeriods: ['mon-wed','thu-sun']` для itemIds;
  обидві дії додають `orderId` до `silpoOrdersHandled`.
- UI `SilpoOrderBanner` над списком: «Схоже, ви оформили замовлення №… у Сільпо. Відмітити N
  продуктів купленими?» з кнопками «Відмітити» і «Ні». Момент «оплачено» = існування замовлення,
  не статус доставки.
- Вебхуків у Сільпо немає; кронів не додаємо, лише читання при відкритті сторінки.
