...
| GET             | `/api/silpo/product?slug&branchId&deliveryType&timeslotStart&timeslotEnd` | Картка товару (`silpo_get_product_details`): галерея, опис, склад, БЖВ → `SilpoProductDetails` |
...
- POST ідемпотентний — повторний скретч **не** перезапускає таймер; згоріла знижка не
  відновлюється, доки жива кука. Перезапуск (для тестів): видалити куку в DevTools
  або інкогніто.
- `liqpay/checkout` бере ціну через `getPlanPrice(planId, discounted)` за кукою
  (+60 с люфту на межі вікна). Після 10 хв — **повні ціни** (рішення узгоджено).
- `liqpay/callback`: поріг anti-tampering тепер `discountAmount` (інакше легітимні
  знижкові оплати позначались би failed); в аналітику йде фактично сплачена сума
...
`magic-link/consume` (тип PlanId), `payment/plan/page.tsx`, `LoaderScreen.tsx`
...
### Доповнення того ж дня

- **Деталі товару:** тап по фото в превʼю відкриває в шторці збільшене фото (галерея з `images`), ціну і картку товару з `silpo_get_product_details` (реальні поля на проді: «Склад», «Країна», «Продавець», «Енергетична цінність (кКал/кДЖ)», «Білки/Жири/Вуглеводи (г)»). Контекст філії/слоту приходить у відповіді `/match`, тож деталі — один виклик Сільпо.
- **«Кошик Сільпо»** на `/shopping-list`: поряд із «Замовити в Сільпо (N)» (або окремо, коли додавати вже нічого) — лінк, що відкриває застосунок Сільпо (Universal Link / `intent://`) або сайт.
