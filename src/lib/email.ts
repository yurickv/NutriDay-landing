...
export async function sendMagicLinkEmail(to: string, token: string) {
  ...
  const subject = 'Ваш магічний лінк для входу в Sytno';
  const html = `
    <div style="font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; line-height: 1.5;">
      <h2>Вітаємо в Sytno!</h2>
      <p>Натисніть кнопку нижче, щоб увійти до свого кабінету без пароля.</p>
      <p style="margin: 24px 0;">
        <a href="${magicLinkUrl}" style="display:inline-block;padding:10px 18px;border-radius:999px;background:#c67139;color:#fff;text-decoration:none;font-weight:600;">
          Увійти одним кліком
        </a>
      </p>
